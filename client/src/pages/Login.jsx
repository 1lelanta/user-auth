...
const Login = () => {
    ...
    const [state, setState] = useState('Sign Up') // fixed casing
    ...
    sm:px-0 bg-gradient-to-br from-blue-200 to-purple-400'>
        ...
        sm:w-96 text-indigo-300 text-sm'> {/* fixed text:sm */}
                <h2 className='text-3xl font-semibold text-white text-center mb-3'>{state === 'Sign Up' ? 'Create account' : 'Login'}</h2>

                <p className='text-center text-sm mb-6'>{state === 'Login' ? 'login to your account' : 'Create your account'}</p>
                ...
                rounded-full bg-[#333A5C]'>
                        ...
                        <input onChange={e => setEmail(e.target.value)}
                            value={email} className='bg-transparent outline-none' type="email" placeholder='Email' required />
                    </div>
                    <div className='mb-4 flex items-center gap-3 w-full px-5 py-2.5
                rounded-full bg-[#333A5C]'>
                    ...
                    <button type="submit" className='w-full py-2.5 rounded-full bg-gradient-to-r
