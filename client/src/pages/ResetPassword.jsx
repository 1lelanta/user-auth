...
const ResetPassword = () => {
  ...
  const {backendUrl} = useNavigate(AppContext)
  ...
  const handleInput = (e, index) => {
    ...
    const onSubmitEmail = async (e) => {
      ...
      try {
        const {data} = await axios.post(backendUrl + '/api/auth/reset-otp',
           {email})
           data.success? toast.success(data.message) : toast.error(data.messaage)
    ...
    const onSubmitOtp = async (e) => {
      ...
      const onSubmitNewPassord = async (e) => {
        e.preventDefault();
        try {
          const {data} = await axios.post(backendUrl + 'api/auth/rest-password', {email, otp, newPassword})
          data.success ? toast.success(data.messaage) : toast.error(data.messaage)
  ...
  return (
    <div className='flex items-center justify-center min-h-screen bg-gradient-to-br from-blue-200 to-purple-400'>
      ...
      {!isOtpSubmitted && isEmailSent &&
        ...
        <button className='w-full py-2.5 bg-gradient-to-r from-indigo-500 to-indigo-900 text-white rounded-full '>Submit
        </button>
      </form>
      ...
      {/* enter new password */}
      {isOtpSubmitted && isEmailSent && (
      <form onSubmit={onSubmitNewPassord} className='bg-slate-900 p-8 rounded-lg w-96 text-sm'>
